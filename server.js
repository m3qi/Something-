...
const CHAT_ID = process.env.CHAT_ID || 'CHAT_ID_HERE';

async function sendTelegramMessage(message) {
    if (BOT_TOKEN === '8947928608:AAFVoWeordXH-h7AzmPBwww3UV-tCTZc_c0') return;
...
app.get('/api/next-stage-time', (req, res) => {
    const currentId = getCurrentOpenStage();
    if (currentId >= stages.length) {
        return res.json({ finished: true });
    }
    const nextStage = stages.find(s => s.id === currentId + 1);
    const openTime = progress.startTime + nextStage.openAfterHours * 60 * 60 * 1000;
    const remainingMs = Math.max(0, openTime - Date.now());
    res.json({
...
app.get('/api/can-access-stage/:stageId', (req, res) => {
    const targetId = parseInt(req.params.stageId);
    const currentId = getCurrentOpenStage();
    res.json({ canAccess: targetId <= currentId });
});
